...
# Vendored from https://github.com/yinboc/liif (models/rdn.py); registry made local.
...
class RDB(nn.Module):
    def __init__(self, growRate0, growRate, nConvLayers, kSize=3):
        ...
        G = growRate
        C = nConvLayers
        convs = [RDB_Conv(G0 + c * G, G) for c in range(C)]
...
class RDN(nn.Module):
    def __init__(self, args):
        ...
        self.RDBs = nn.ModuleList(RDB(growRate0=G0, growRate=G, nConvLayers=C) for _ in range(self.D))
