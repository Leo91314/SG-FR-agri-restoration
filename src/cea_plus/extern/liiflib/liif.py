# LIIF decoder (Chen et al., CVPR 2021). Vendored from https://github.com/yinboc/liif
# (models/liif.py); registry made local and .cuda() replaced with device-agnostic ops.
import torch
import torch.nn as nn
import torch.nn.functional as F

from . import register, make, make_coord
...
class LIIF(nn.Module):
    ...
    def query_rgb(self, coord, cell=None):
        ...
        ry = 2 / feat.shape[-1] / 2
        feat_coord = (
            make_coord(feat.shape[-2:], flatten=False, device=feat.device)
            .permute(2, 0, 1)
            .unsqueeze(0)
